...
def process_file(org, file, pred_path, th, out_path):
    result = {
        'has_pockets': 0,
        'pockets': [],
        'num_pockets': 0,
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if result['has_pockets']:
                ...
                num += result["num_pockets"]    
            # except Exception as exc:
            #     print(f'{file} generated an exception: {exc}')
                                                                                                                                                                                                                                                                                                    
        return results, num, num_prot, plddt_all
...
# Get the net charge of a pocket sequence
...
if __name__ == "__main__":
    ...
    ORGANISMS = ["ECOLI", "YEAST", "CANAL", "ARATH", "ORYSJ", "MAIZE", "SOYBN", "CAEEL", "DROME", "MOUSE", "HUMAN"]
    ORG_TITLES = [f"<b>{org}</b>" for org in ORGANISMS]
    TH = 0.5
    THREADS = 50
    STRUC_PATH = "../../data/proteins"
    PRED_PATH = "../../data/prank"
    POCKET_PATH = "../../data/pockets"
    PAE_PATH = "../../data/pae"
    OUT_PATH = "../../results"
    PLOT_DIR = "../../results/pictures/REVISION"
    FRAG_PATH = "../../../Pocketeome/db_files"
    DESC_PATH = "../../results/pocket_desc"
    PAE_TH = 10 
    PLDDT_TH = 70
    PLDDT_DATA = {}
    POCKET_DATA = {}
    ...
    if not os.path.isdir(f"{OUT_PATH}/pictures"):
        os.mkdir(f"{OUT_PATH}/pictures")

    # Evaluate predicted binding sites for each organism
    for org in ORGANISMS:
        print(f"Evaluating predicted binding sites for {org}")    
        pae_path = f"{PAE_PATH}/{org}"
        struc_path = f"{STRUC_PATH}/{org}"
        pred_path = f"{PRED_PATH}/{org}"
        pocket_path = f"{POCKET_PATH}/{org}"
        out_file = f"{OUT_PATH}/bind_dict/{org}_pockets.pkl"
        tm_dict = pickle.load(open(f"{DESC_PATH}/{org}/{org}_tm_info.pkl", "rb"))
        if not os.path.isfile(f"{FRAG_PATH}/{org}/frag_df.tsv.gz"): 
            fragments = []
        else:
            fragments = pd.read_csv(f"{FRAG_PATH}/{org}/frag_df.tsv.gz", delimiter="\t")
            fragments = fragments["Entry"].to_list()

        print(f"Parameters\nStructures: {struc_path}\nPrank predictions: {pred_path}\nPrank th: {TH}\nOutput: {pocket_path}, {out_file}")
    

        results, n_pockets, n_prot, plddt_all = process_files_in_parallel(struc_path, pred_path, TH, pocket_path, org, max_workers=THREADS)
        ...
        print(f"Found {n_pockets} pockets for {org}")
        NUM_DICT[org] = {"n_prot": n_prot, "n_pockets": n_pockets}
        #with open(out_file, "wb") as f:
        #    pickle.dump(results, f)
    
    # Plot pLDDT and PAE data for binding sites
    pocket_df = pd.DataFrame.from_dict(POCKET_DATA, orient='index')
    pocket_df.rename(columns={"organism": "organism", "plddt": "pLDDT", "pae": "PAE"}, inplace=True)
    median_list = []
    plddt_list = []
    pae_list = []
    for org in ORGANISMS: 
        tmp = pocket_df[pocket_df["organism"] == org] 
        ...
        num_pae = len(tmp_pae)
        n_prot = NUM_DICT[org]["n_prot"]
        n_pockets = NUM_DICT[org]["n_pockets"]
        median_list.append({"org": org, "n_prot": n_prot, "n_pockets": n_pockets, "pocket/protein": round(n_pockets/n_prot, 3),
                            "plddt_med": plddt_median, "pae_med": pae_median, "num_before": num_before,
                            "num_below_plddt": num_plddt, "num_above_pae": num_pae})
    median_df = pd.DataFrame.from_records(median_list)
    ...
    thresholds = {'pLDDT': 70.0, 'PAE': 10} 


    # Create subplots
    # Set DPI and figure size in pixels
    dpi = 300
    fig_width_px = 2250
    fig_height_px = 1400
    figsize_in = (fig_width_px / dpi, fig_height_px / dpi)
    fig, axes = plt.subplots(1, 2, figsize=figsize_in, dpi=dpi)
    subplot_labels = ["A", "B"]
    for ax, label, (prop, threshold) in zip(axes, subplot_labels, thresholds.items()):
        sns.violinplot(x='organism', y=prop, data=pocket_df, ax=ax, inner='box', color='lightgray', linecolor='black')
    ...
    n_row = 4
    n_col = 3
    fig, axes = plt.subplots(n_row, n_col, figsize=figsize_in, dpi=dpi)
    ...
    # Iterate through organisms and plot violin plots
    med_dict = {}
    for org in ORGANISMS:
        tmp = plddt_df[plddt_df["org"] == org]
        ...
        print(f"{org} - With BS: {med_dict[org]['with_bs']}, Without BS: {med_dict[org]['without_bs']}")
        ...
        axes[row, col].set_title(f"{org}", fontsize=12, fontweight="bold")
    ...
    axes[3, 2].set_visible(False)  # Hide the last empty subplot
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    fig.savefig(f"{PLOT_DIR}/prot_plddt.tif", dpi=300)

    median_df["prot_with_plddt"] = [med_dict[org]["with_bs"] for org in median_df["org"]]
    median_df["prot_without_plddt"] = [med_dict[org]["without_bs"] for org in median_df["org"]]
    median_df.to_csv(f"{PLOT_DIR}/pocket_median.csv", index=False)
